...
Основные параметры по-прежнему задаются в исходниках:

- `appsettings.json`
  - `Update` - API обновлений, таймауты, user-agent;
  - `Monitoring` - интервалы и таймауты ping/опроса;
  - `Hosts` - параметры сброса hosts.
- `Config/regions.json` - каталог регионов и их host-эндпоинты.
- `Config/default-hosts.txt` - шаблон hosts для сброса.

В релизной сборке (`single-file + self-contained`) эти файлы вшиваются в `PingByDaylight.exe`.
Приложение не зависит от внешних копий рядом с `exe` и не использует file override.
